...
class HeaderLocationSearch extends Component {
    ...
    onChange(value) {
        const { setPersonalizedValues, change, submitForm, page } = this.props;
        let location;
        let updatedURI, uri = '/s';
        ...
        if (['/s', '/store'].includes(location) && !value) {
            setPersonalizedValues({ name: 'location', value: "" });
    ...
    render() {
        const { className, containerClassName, personalized, page} = this.props;
        ...
        return (
            <div className={'headerSearch'}>
                <div className={cx(c.displayTable, c.searchContainer)}>
                    ...
                    {page === 'search' ? (<div className={c.displayTableCell}>
                            ...
                            render={googleMaps =>
                                googleMaps && (
                                    <Geosuggest
                                        ...
                                        placeholder={page === 'search' ? formatMessage(messages.homeWhere) : formatMessage(messages.storeWhere)}
                    ...
                    {/* For the cellar searches */}
                    {page === 'storeSearch' ? (<div className={c.displayTableCell}>
                        <ReactGoogleMapLoader
                            params={{
                                key: googleMapAPI, // Define your api key here
                                libraries: "places", // To request multiple libraries, separate them with a comma
                            ...
                            render={googleMaps =>
                                googleMaps && (
                                    <Geosuggest
                                        ref={el => this._geoSuggest = el}
                                        placeholder={formatMessage(messages.storeWhere)}
                                        inputClassName={className}
                                        className={containerClassName}
                                        initialValue={locationValue}
                                        onChange={this.onChange}
