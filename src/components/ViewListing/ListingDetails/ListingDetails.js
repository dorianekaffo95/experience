...
class ListingDetails extends React.Component {
  ...
  getPrivateOrCollectiveText(value) {
    console.log(`Value sent: ${value}`);
    switch(value) {
      case 'PRIVATE': return messages.private;
      case 'COLLECTIVE': return messages.collective;
      case 'PRIVATE_COLLECTIVE':
      default: return messages.privateCollectiveOption;
    }
  }
  ...
  render() {
    ...
    let sharedSpaces = [],
      ...
      bedTypes = [],
      listBedTypes = [];
    let description, personCapacity, bathrooms, bedrooms, beds, privateOrCollective, visitWithOwner, familyWelcome, instantBooking;
    ...
    if (data.settingsData != undefined && data.settingsData.length > 0) {
      propertyType = checkValue(
        data.settingsData && data.settingsData[1].listsettings.itemName,
        ""
      );
      roomType = checkValue(
        data.settingsData && data.settingsData[0].listsettings.itemName,
        ""
      );
    }
    ...
    // -- New fields
    experienceTypes = checkValue(data.experienceType, []);
    plusesIncluded = checkValue(data.plusesIncluded, []);
    spokenLanguages = checkValue(data.spokenLanguages, []);
    privateOrCollective = checkValue(data.privateOrCollective, "");
    visitWithOwner = checkValue(data.visitWithOwner, "");
    familyWelcome = checkValue(data.familyWelcome, false);
    ...
    let privateOrCollectiveValue;
    if (privateOrCollective) {
      privateOrCollectiveValue = this.getPrivateOrCollectiveText(privateOrCollective);
    }

    console.log("User amenities: ", userAmenities);
    ...
    return (
      <Row className={cx(s.pageContent)}>
            ...
            <div className={cx(s.spaceTop3, s.space4)}>
              <p>
                <a
                  ...
                  onClick={() => contactHostOpen(data.id)}
                >
                ...
                <Col md={12} lg={12}>
                  ...
                  <p className={s.splitList}>
                    <span className={cx(s.text)}>
                      ...
                      <strong>{checkIn(checkInStart, checkInEnd)}</strong>
                    </span>
                  </p>
                  <p className={s.splitList}>
                    <span className={cx(s.text)}>
        ...
        {/* Pluses included */}
        {
          plusesIncluded && plusesIncluded.length > 0 && <div>
            {" "}
            <ListItem
              itemList={plusesIncluded}
              label={formatMessage(messages.plusesIncluded)}
        ...
        {
          privateOrCollective && <div>
            {" "}
            <ListSingleItem
              label={formatMessage(messages.privateExperience)}
              value={formatMessage(privateOrCollectiveValue)}
            />
            <div>
              <hr />
            </div>
        ...
        {/* Family Welcome */}
        {
          familyWelcome && <div>
            {" "}
            <ListSingleItem
              label={formatMessage(messages.familyWelcome)}
              value={formatMessage(messages[familyWelcome.toLowerCase()])}
            />
            <div>
              <hr />
            </div>
        ...
        {/* Instant Booking */}
        {
          instantBooking && <div>
            {" "}
            <ListSingleItem
              label={formatMessage(messages.instantBook)}
              value={instantBook}
            />
            <div>
              <hr />
        ...
        {/* Owner Care */}
        {
          visitWithOwner && <div>
            {" "}
            <ListSingleItem
              label={formatMessage(messages.ownerCare)}
              value={visitWithOwner ? formatMessage(messages.ownerTakesCare) : formatMessage(messages.ownerNoCare)}
            />
            <div>
              <hr />
            </div>
