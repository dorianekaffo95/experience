...
class Dates extends Component {
  ...
  async handleSubmit() {
    ...
    let endDate;
    console.log("Dates: ", personalized);

    if (personalized && personalized.startDate) {
      change('startDate', `${personalized.startDate}`);
    }
    if (personalized && personalized.endDate) {
      change('endDate', `${personalized.endDate}`);
    }
  ...
  render() {
    ...
    let buttonLabel = isExpand ? formatMessage(messages.arrival) + ' - ' + formatMessage(messages.departure) : formatMessage(messages.dates);
    let isActive = false;
    if (personalized) {
    ...
    return (
      <div className={className}>
        <div ref={this.setBtnWrapperRef}>
          <Button
            className={cx({ [s.btnSecondary]: (isExpand === true || isActive == true) }, s.btn, s.btnFontsize, s.responsiveFontsize)}
