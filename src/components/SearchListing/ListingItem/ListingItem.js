          ...
          <a className={s.listInfoLink} href={"/rooms/" + formatURL(title) + '-' + id} target={"_blank"}>
            <Row>
              ...
              <Col xs={12} sm={12} md={12} className={cx(s.textEllipsis)} style={{color: "#484848"}}>
                {experienceTypes && experienceTypes[0] && formatMessage(messages[`listSetting${experienceTypes[0].listsettings.id}ItemName`])} {experienceTypes && experienceTypes.length > 1 && formatMessage(messages.andMore)} - {`${duration} h`}
              </Col>
