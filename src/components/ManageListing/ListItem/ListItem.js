...
class ListItem extends React.Component {
  ...
  title(data) {
    if (data) {
      if (data.title != null) {
        ...
        return data.settingsData[0] && data.settingsData[0].listsettings.itemName + " in " + data.city;
      }
    }
  }
  ...
  render() {
    ...
    let updatedDate = moment(data.updatedAt).locale(locale).format('Do MMMM YYYY');
    let listId = data.id;
