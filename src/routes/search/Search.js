...
class Search extends React.Component {
  ...
  componentWillMount() {
    const { getListingFields: i } = this.props;
    // Get listing settings fields data
    console.log(
      "Search Component --- GET LISTING FIELDS",
      getListingFields,
      i()
    );
    i();
    console.log("Search Component After --- GET LISTING FIELDS");
  }
  ...
  mobileNavigation() {
    ...
    if (searchResults) {
      leftNav = (
        <Button
          className={cx(s.filterButton, s.locationBtn)}
          bsStyle="link"
          onClick={() => showMap()}
        >
    ...
    if (searchMap) {
      leftNav = (
        ...
        >
          ...
          <Material.MdSettingsInputComposite className={s.icon} />
        </Button>
      );
      rightNav = (
        <Button
      ...
      <div className={cx(s.root, "searchPage")}>
        <div className={s.container}>
          ...
          {!smallDevice && isMapShow && (
            <div className={cx(s.searchMapContainer)}>
