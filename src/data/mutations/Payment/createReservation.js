...
// Sequelize models
import { Reservation, ReservationSpecialPricing, ExperienceHours } from '../../models';
...
const createReservation = {
  ...
  args: {
    ...
    experienceHourId: {type: IntType},
    adultPrice: {type: new NonNull(FloatType)},
    teenPrice: {type: new NonNull(FloatType)},
    childOrYoungerPrice: {type: new NonNull(FloatType)}
  },
  ...
  async resolve({ request, response }, {
    ...
    if (request.user && !request.user.admin) {
      ...
      // -- Get corresponding experienceHour
      const selectedHour = await ExperienceHours.findOne({
        where: {
          id: experienceHourId,
          status: 'available'
        }
      ...
      // -- In case no experience hour is selected
      if (!selectedHour) {
        return {
      ...
      // -- Check if there is enough guests
