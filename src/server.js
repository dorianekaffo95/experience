...
process.env["NODE_TLS_REJECT_UNAUTHORIZED"] = 0; // Needed to be rejected because
//
...
app.get('*', async (req, res, next) => {
  try {
    ...
    if (!req.user) {
      ...
      // if (["/login", "/register"].includes(currentLocation) || currentLocation.startsWith("/siteadmin")) {
